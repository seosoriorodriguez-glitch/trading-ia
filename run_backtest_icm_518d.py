# -*- coding: utf-8 -*-
"""
Backtest LIMIT ORDERS — US30 M5/M1 — 518 dias IC Markets
Data: data/US30_icm_M5_518d.csv + data/US30_icm_M1_500k.csv
Configuracion identica al bot live (DEFAULT_PARAMS).
"""
import sys
...
OUT_FILE = "strategies/order_block/backtest/results/ob_icm_518d.csv"

print("=" * 60)
print("  BACKTEST LIMIT ORDERS — US30 ICM — 518 dias")
...
print(f"  M1: {len(df_m1):,} velas ({df_m1['time'].iloc[0].date()} -> {df_m1['time'].iloc[-1].date()})")

m1_start = df_m1["time"].iloc[0]
m1_end   = df_m1["time"].iloc[-1]
days = (m1_end - m1_start).days
print(f"  Periodo efectivo: {days} dias ({m1_start.date()} -> {m1_end.date()})")
...
long_wr_n  = len(df_results[(df_results["direction"]=="long")  & (df_results["pnl_usd"]>0)])
short_wr_n = len(df_results[(df_results["direction"]=="short") & (df_results["pnl_usd"]>0)])
long_wr    = long_wr_n  / n_long  * 100 if n_long  > 0 else 0
short_wr   = short_wr_n / n_short * 100 if n_short > 0 else 0
...
print("\n" + "=" * 60)
print("  RESULTADOS — BACKTEST ICM 518 DIAS")
print("=" * 60)
