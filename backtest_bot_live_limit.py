# -*- coding: utf-8 -*-
"""
Backtest con la configuración EXACTA del bot live ACTUAL (con lógica LIMIT).
...
def main():
    print("=" * 80)
    print("BACKTEST - BOT LIVE ACTUAL CON LOGICA LIMIT")
    print("=" * 80)
    print()
    ...
    print(f"   min_impulse_pct: {params['min_impulse_pct']}")
    ...
    # Ejecutar backtest
    print("Ejecutando backtest con logica LIMIT...")
    ...
    print(f"Trades/dia:         {len(df_trades) / duration_days:.1f}")
    print()
    ...
    # Guardar resultados
    output_file = "backtest_bot_live_limit_results.csv"
    df_trades.to_csv(output_file, index=False)
    print(f"Resultados guardados en: {output_file}")
    print()
    
    # Exportar últimas 10 ganadoras
    print("=" * 80)
    print("ULTIMAS 10 OPERACIONES GANADORAS")
    print("=" * 80)
    ...
    print()
    for idx, trade in last_10_winners.iterrows():
        print(f"Trade #{int(trade['trade_id'])} - {trade['direction'].upper()}")
        print(f"  Fecha:  {trade['entry_time_chile']}")
        print(f"  Entry:  {trade['entry_price']:.2f}")
        print(f"  SL:     {trade['sl']:.2f}")
        print(f"  TP:     {trade['tp']:.2f}")
        print(f"  Exit:   {trade['exit_price']:.2f}")
        print(f"  PnL:    ${trade['pnl_usd']:,.2f} ({trade['pnl_r']:.2f}R)")
        print()
    ...
    print("=" * 80)
    print()
    print(f"ESTRATEGIA: Order Block M5/M1 con ordenes LIMIT")
    print(f"PERIODO:    {duration_days} dias ({start_date.date()} a {end_date.date()})")
    print()
    print(f"RENTABILIDAD:  {total_return:+.2%}")
    print(f"MAX DD:        {max_dd_pct:.2f}%")
    print(f"WIN RATE:      {win_rate:.1%}")
    print(f"PROFIT FACTOR: {profit_factor:.2f}")
    print()
    print(f"LONG:  {len(df_long)} trades, ${df_long['pnl_usd'].sum():+,.2f}")
    print(f"SHORT: {len(df_short)} trades, ${df_short['pnl_usd'].sum():+,.2f}")
    print()
    print(f"CUMPLE FTMO: {'SI' if max_dd_pct > -10 else 'NO'} (DD < 10%)")
    print()
