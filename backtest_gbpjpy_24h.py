# -*- coding: utf-8 -*-
"""
Backtest GBPJPY 24h - Sin filtro de sesion.
...
DATOS: Solo ultimos 29 dias donde M1 y M5 estan disponibles.
"""
...
def main():
    print("=" * 80)
    print("BACKTEST GBPJPY 24H - ESTRATEGIA ORDER BLOCK LIMIT")
    print("=" * 80)
    print()
    ...
    # Necesitamos ~100 velas M5 previas para deteccion de OB
    m5_start = m1_start - timedelta(days=7)  # 7 dias extra para OB detection
    ...
    params["avg_spread_points"] = 0.002  # 2 pips
    params["point_value"] = 1000.0  # $1000 por pip
    
    # DESACTIVAR filtro de sesion (24h trading)
    # Crear sesion 24h para que is_session_allowed retorne True
    params["sessions"] = {
        "full_day": {
            "start": "00:00",
            "end": "23:59",
    ...
    # Ejecutar backtest
    print("Ejecutando backtest GBPJPY 24H...")
    print()
    ...
    print("=" * 80)
    print("RESULTADOS GENERALES - GBPJPY 24H")
    print("=" * 80)
    ...
    print("=" * 80)
    print("COMPARACION: GBPJPY 24H vs US30 NY")
    print("=" * 80)
    print()
    print(f"                    GBPJPY 24H      US30 NY")
    ...
    # Guardar resultados
    output_file = "backtest_gbpjpy_24h_results.csv"
    ...
    for idx, trade in df_last15.iterrows():
        ...
        print(f"{trade['entry_time']} | {trade['direction'].upper():5} | {resultado:4} | "
              f"Entry: {trade['entry_price']:.3f} | SL: {trade['sl']:.3f} | TP: {trade['tp']:.3f} | "
    ...
    df_last15_sorted.to_csv("gbpjpy_ultimas_15_trades.csv", index=False)
    print("Ultimas 15 operaciones guardadas en: gbpjpy_ultimas_15_trades.csv")
    print()
    ...
    if total_return > 0.20:
        print("EXCELENTE: Estrategia muy rentable en GBPJPY 24h")
    elif total_return > 0.10:
        print("BUENO: Estrategia rentable en GBPJPY 24h")
    elif total_return > 0:
        print("MARGINAL: Estrategia levemente rentable en GBPJPY 24h")
    else:
        print("NO RENTABLE: Estrategia pierde en GBPJPY 24h")
