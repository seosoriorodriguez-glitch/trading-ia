...
Para activar: python strategies/order_block_gold/live/run_bot.py --balance 10000
"""
import sys
import time
import yaml
...
class OrderBlockGoldBot:
    ...
    def __init__(
        ...
        self.ob_monitor   = LiveOBMonitor(GOLD_PARAMS, self.data_feed)
    ...
    def start(self):
        print("Iniciando Order Block ORO (XAUUSD) Bot...", flush=True)
        _sess = GOLD_PARAMS["sessions"]["london"]
        print(
            f"Sesion: London {_sess['start']}-{_sess['end']} UTC+3 "
            f"(skip {_sess['skip_minutes']}m) | RR={GOLD_PARAMS['target_rr']} "
            f"| Buffer={GOLD_PARAMS['buffer_points']}",
            flush=True,
        )
    ...
    def _main_loop(self):
        while self.running:
            ...
            if Path("STOP_GOLD.txt").exists():
                print("STOP_GOLD.txt detectado - deteniendo bot")
                self.stop()
                break
