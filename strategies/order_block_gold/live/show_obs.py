# -*- coding: utf-8 -*-
"""
Muestra los Order Blocks activos que el Bot ORO (XAUUSD) detectaria en este momento.

Uso:
    python strategies/order_block_gold/live/show_obs.py

Conecta a MT5 (instancia MT5_FVG), descarga las ultimas 350 velas M5,
detecta OBs con GOLD_PARAMS, aplica expiry/destruccion y muestra los FRESH.
"""
import sys
...
from strategies.order_block_gold.live.ob_monitor   import LiveOBMonitor
...
def main():
    print("=" * 70)
    print("  OB SCANNER — Bot ORO  XAUUSD  London  (GOLD_PARAMS)")
    print("=" * 70)

    feed = LiveDataFeed("XAUUSD")
    if not feed.connect():
        return
    ...
    monitor = LiveOBMonitor(GOLD_PARAMS, feed)
    ...
    if df_m5 is not None and len(df_m5) > 0:
        ...
        in_session = is_session_allowed(last_time, GOLD_PARAMS)
        _s = GOLD_PARAMS["sessions"]["london"]
    ...
    if price:
        ...
        buf       = GOLD_PARAMS["buffer_points"]
        target_rr = GOLD_PARAMS["target_rr"]
