# -*- coding: utf-8 -*-
"""
Corre la estrategia OB de ORO sobre los ULTIMOS ~60 dias de DE40 tomados del
...
Uso (en el VPS):
    python strategies/order_block_dax/live/recent_backtest.py
"""
import sys, argparse
...
print(f"DE40 del terminal FVG: M5={len(df5)} velas, M1={len(df1)} velas")
print(f"Rango: {df1.time.iloc[0]} -> {df1.time.iloc[-1]}")

params = dict(DAX_PARAMS)
