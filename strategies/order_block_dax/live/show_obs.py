# -*- coding: utf-8 -*-
"""
Muestra los Order Blocks activos que el Bot ORO (DE40) detectaria en este momento.

Uso:
    python strategies/order_block_dax/live/show_obs.py

Conecta a MT5 (instancia MT5_FVG), descarga las ultimas 350 velas M5,
detecta OBs con DAX_PARAMS, aplica expiry/destruccion y muestra los FRESH.
"""
import sys
...
from strategies.order_block_dax.live.ob_monitor   import LiveOBMonitor
...
def main():
    print("=" * 70)
    print("  OB SCANNER — Bot ORO  DE40  London  (DAX_PARAMS)")
    print("=" * 70)

    feed = LiveDataFeed("DE40")
    if not feed.connect():
        return
    ...
    monitor = LiveOBMonitor(DAX_PARAMS, feed)
    ...
    if df_m5 is not None and len(df_m5) > 0:
        ...
        in_session = is_session_allowed(last_time, DAX_PARAMS)
        _s = DAX_PARAMS["sessions"]["london"]
    ...
    if price:
        ...
        buf       = DAX_PARAMS["buffer_points"]
        target_rr = DAX_PARAMS["target_rr"]
