# -*- coding: utf-8 -*-
"""
Order Executor - Ejecucion de ordenes en MT5 para el bot ORO (DE40).
...
correcto en DE40 ($100 por 1.0 de precio) y en cualquier otro simbolo.
"""
...
class OrderExecutor:
    ...
    MAGIC = 345683  # NY=345678 | BTC=345679 | London=345680 | Darwinex=345681 | ORO=345682 | DAX=345683

    def __init__(self, symbol: str = "DE40"):
        self.symbol = symbol

    def _usd_per_point(self) -> float:
        """USD que gana/pierde 1 lote por cada 1.0 de movimiento de precio.
        Para DE40 ~= 100; para US30.cash FTMO = 1.0.
        ...
        ORDEN de fuentes (robusto contra tick_value=0 que reporta algun broker):
          ...
          3) trade_contract_size      (DE40 USD = 100)
        ...
        if tick is not None:
            ...
            if price:
                ...
                if prof:
                    return abs(prof)
        # 3) contract_size (para DE40 en USD = 100 oz -> $100 por 1.0)
    ...
    def execute_signal(self, signal, risk_usd: float, dry_run: bool = False) -> Tuple[bool, dict]:
        ...
        request = {
            ...
            "deviation":   0,
            "magic":       self.MAGIC,
            "comment":     f"OBDE_{trade_type}_STOP",  # OBDE = Order Block Gold
