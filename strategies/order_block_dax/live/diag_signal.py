...
Uso:  python strategies/order_block_dax/live/diag_signal.py
...
# --- gate 1: sesion ---
sess_ok = is_session_allowed(srv_now, DAX_PARAMS) if srv_now else False
print(f"\n[1] SESION permitida ahora?  {sess_ok}   (sesiones: {list(DAX_PARAMS['sessions'].keys())})")
...
# --- gate 3: OBs activos ---
mon = LiveOBMonitor(DAX_PARAMS, feed)
n = mon.update_obs()
print(f"\n[3] OBs activos: {n}")
