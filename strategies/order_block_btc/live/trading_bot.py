# -*- coding: utf-8 -*-
"""
Trading Bot - Orquestador principal de la estrategia Order Block BTCUSD.

Opera 24/7: sin restriccion de horario, BTC nunca cierra.
Proceso completamente independiente del bot US30.
...
Para activar: python strategies/order_block_btc/live/run_bot.py [--dry-run]
"""
import sys
import time
import yaml
...
from strategies.order_block_btc.live.data_feed      import LiveDataFeed
from strategies.order_block_btc.live.ob_monitor     import LiveOBMonitor, _ob_key
from strategies.order_block_btc.live.order_executor import OrderExecutor
from strategies.order_block_btc.live.risk_manager   import FTMORiskManager
from strategies.order_block_btc.live.monitor        import TradingMonitor
from strategies.order_block_btc.backtest.config     import BTC_PARAMS


class OrderBlockBotBTC:
    """Bot de trading Order Block para BTCUSD - 24/7."""
    ...
    def __init__(
        ...
        self.ob_monitor   = LiveOBMonitor(BTC_PARAMS, self.data_feed)
    ...
    def start(self):
        print("Iniciando Order Block Bot BTC...", flush=True)
        ...
        self.running = True
        print("Bot BTC activo 24/7. Ctrl+C para detener.", flush=True)

        try:
            self._main_loop()
        except KeyboardInterrupt:
            print("\nDeteniendo bot BTC...", flush=True)
            self.stop()
    ...
    def stop(self):
        ...
        print("Bot BTC detenido.", flush=True)
    ...
    def _main_loop(self):
        while self.running:
            ...
            # Emergency stop file exclusivo para BTC
            if Path("STOP_BTC.txt").exists():
                print("STOP_BTC.txt detectado - deteniendo bot BTC")
                self.stop()
                break
            ...
            # Verificar senales cada vela M1
            if now.second < 10:
    ...
    def _on_trade_closed(self, ticket: int, trade_info: dict):
        try:
            ...
            exit_reason = "sl_hit" if abs(exit_price - sl) < 5 else "tp_hit"  # BTC: tolerancia 5 pts
