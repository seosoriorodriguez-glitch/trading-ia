# -*- coding: utf-8 -*-
"""
Monitor y Logger - Dashboard en consola y log de trades para BTCUSD.
"""
import csv
import os
import threading
...
class TradingMonitor:
    """Logging de eventos, trades y dashboard de consola para BTCUSD."""

    TELEGRAM_TOKEN   = "8577007615:AAHy31IegzvbezCpyNfIlaZh_IsKuV-4M9A"
    TELEGRAM_CHAT_ID = "6265548967"

    def __init__(self, logs_dir: str = "logs_ob_btc",
    ...
    def log_trade_opened(self, trade_info: dict):
        ...
        self._log_event(msg)
        if self.telegram:
            direction = "BUY" if trade_info['type'] == "LONG" else "SELL"
            tg = (
                f"<b>TRADE ABIERTO - OB BTC Bot</b>\n"
                f"{direction} BTCUSD\n"
    ...
    def log_trade_closed(self, trade_info: dict):
        ...
        if self.telegram:
            result_txt = "WIN" if pnl_usd >= 0 else "LOSS"
            tg = (
                f"<b>TRADE CERRADO [{result_txt}] - OB BTC Bot</b>\n"
                f"{trade_info['type']} BTCUSD\n"
                f"Entry: {trade_info['entry_price']:.2f}  ->  Exit: {trade_info['exit_price']:.2f}\n"
                f"PnL: <b>{sign}${pnl_usd:,.2f}</b>  ({r_mult:+.2f}R)\n"
                f"Razon: {trade_info.get('exit_reason', '?')}  |  Duracion: {trade_info.get('duration_minutes', 0):.0f} min"
            )
            self.telegram.send(tg)
    ...
    def log_risk_alert(self, alert_type: str, message: str):
        ...
        if self.telegram:
            self.telegram.send(f"[ALERTA RIESGO - OB BTC Bot]\n{alert_type}: {message}")
    ...
    def print_dashboard(self, status: dict):
        ...
        print("=" * 55, flush=True)
        print("  ORDER BLOCK BOT - BTCUSD  M5 deteccion / M1 entrada  24/7", flush=True)
        print("=" * 55, flush=True)
