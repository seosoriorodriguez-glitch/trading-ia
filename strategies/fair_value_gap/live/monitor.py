# -*- coding: utf-8 -*-
"""
Monitor y Logger - Dashboard en consola y log de trades para bot FVG.
"""
import csv
import os
import threading
...
class TradingMonitor:
    """Logging de eventos, trades y dashboard de consola para bot FVG."""

    TELEGRAM_TOKEN   = "8577007615:AAHy31IegzvbezCpyNfIlaZh_IsKuV-4M9A"
    TELEGRAM_CHAT_ID = "6265548967"

    def __init__(self, logs_dir: str = "logs_fvg",
    ...
    def log_trade_closed(self, trade_info: dict):
        ...
        if self.telegram:
            ...
            tg = (
                f"<b>{emoji} TRADE CERRADO — FVG Bot</b>\n"
    ...
    def log_risk_alert(self, alert_type: str, message: str):
        ...
        if self.telegram:
            self.telegram.send(f"⚠️ <b>ALERTA RIESGO — FVG Bot</b>\n{alert_type}: {message}")
    ...
    def print_dashboard(self, status: dict):
        ...
        print("=" * 55, flush=True)
        print("  FVG BOT - US30.cash  M5 deteccion / M1 entrada", flush=True)
        print("=" * 55, flush=True)
        ...
        print(f"  Riesgo/trade: 0.5% del balance inicial", flush=True)

        bias = fvgs.get("trend_bias") or "sin filtro"
        print(f"  FVGs activos: {fvgs.get('total', 0)}"
              f"  (bull:{fvgs.get('bullish', 0)} / bear:{fvgs.get('bearish', 0)})"
