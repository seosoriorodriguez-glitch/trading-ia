# -*- coding: utf-8 -*-
"""
Test de orden para validar que el bot FVG puede conectar a MT5
y enviar/cancelar ordenes correctamente con magic 345681.

Uso:
    python strategies/fair_value_gap/live/test_order.py

El script:
  1. Conecta a MT5 (instancia MT5_US30)
  ...
  4. Verifica que la orden aparece con magic 345681
  5. Cancela la orden
...
SYMBOL = "US30.cash"
MAGIC  = 345681
MT5_PATH = r"C:\Program Files\MT5_US30\terminal64.exe"

def main():
    print("=" * 55)
    print("  TEST DE ORDEN — FVG Bot (magic 345681)")
    print("=" * 55)
    ...
    fvg_orders = [o for o in (orders or []) if o.magic == MAGIC]
    ...
    fvg_remaining = [o for o in remaining if o.magic == MAGIC]

    if fvg_remaining:
        print(f"  ADVERTENCIA: Quedan {len(fvg_remaining)} ordenes FVG pendientes!")
        return False

    positions = mt5.positions_get(symbol=SYMBOL) or []
    fvg_positions = [p for p in positions if p.magic == MAGIC]

    if fvg_positions:
        print(f"  ADVERTENCIA: Hay {len(fvg_positions)} posiciones FVG abiertas!")
        return False

    print(f"  OK: Sin ordenes ni posiciones FVG pendientes")
    ...
    print("=" * 55)
    print("\n  El bot FVG puede:")
    print("  - Conectar a MT5 (instancia MT5_US30)")
    print("  - Enviar ordenes STOP con magic 345681")
    print("  - Cancelar ordenes correctamente")
    print("  - Sin interferir con otros bots")
    print(f"\n  Listo para produccion:")
    print(f"  python strategies/fair_value_gap/live/run_bot.py --balance 10000")
    print()
    return True
