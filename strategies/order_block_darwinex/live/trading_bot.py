# -*- coding: utf-8 -*-
"""
Trading Bot - Orquestador principal del bot DARWINEX (WS30).
Copia independiente de order_block_london/live/trading_bot.py — NO comparte
archivos con FTMO. Editar aqui NO afecta a los bots FTMO.
...
Para activar: python strategies/order_block_darwinex/live/run_bot.py --balance 100000
...
from strategies.order_block_darwinex.live.data_feed      import LiveDataFeed
from strategies.order_block_darwinex.live.ob_monitor     import LiveOBMonitor, _ob_key
from strategies.order_block_darwinex.live.order_executor import OrderExecutor
from strategies.order_block_darwinex.live.risk_manager   import FTMORiskManager
from strategies.order_block.live.monitor                 import TradingMonitor
from strategies.order_block_darwinex.backtest.config     import DARWINEX_PARAMS


class OrderBlockDarwinexBot:
    """Bot de trading Order Block para Darwinex Zero (WS30)."""
    ...
    def __init__(
        ...
        self.ob_monitor   = LiveOBMonitor(DARWINEX_PARAMS, self.data_feed)
        self.executor     = OrderExecutor(symbol)
        self.risk_manager = FTMORiskManager(ftmo_cfg, initial_balance)
        self.monitor      = TradingMonitor(logs_dir="logs_darwinex")
    ...
    def start(self):
        print("Iniciando Order Block DARWINEX Bot...", flush=True)
        _sess = DARWINEX_PARAMS["sessions"]["london"]
        print(
            f"Sesion: London {_sess['start']}-{_sess['end']} (hora servidor Darwinex) "
            f"(skip {_sess['skip_minutes']}m) | RR={DARWINEX_PARAMS['target_rr']} "
            f"| Buffer={DARWINEX_PARAMS['buffer_points']}",
            flush=True,
        )
        ...
        if account:
            ...
            # Evita correr tamano de $100k en cuenta equivocada por emparejar mal
            # --balance con --terminal-path.
    ...
    def _main_loop(self):
        while self.running:
            ...
            if Path("STOP_DARWINEX.txt").exists():
                print("STOP_DARWINEX.txt detectado - deteniendo bot")
                self.stop()
                break
    ...
    def _check_weekend_close(self, now: datetime):
        """Cierre de fin de semana (intradia, no swing). El viernes a partir de
    ...
    def _check_news_blackout(self, now: datetime):
        """Filtro de noticias. Durante la ventana ET no se abren trades nuevos
        (lo bloquea can_take_trade) y ademas se CANCELAN las ordenes STOP
        pendientes. Re-chequea cada 10s mientras dure la ventana."""
