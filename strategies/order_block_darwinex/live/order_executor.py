# -*- coding: utf-8 -*-
"""
Order Executor - Ejecucion de ordenes en MT5 para el bot DARWINEX (WS30).
Copia independiente del order_executor de order_block_london (FTMO).
...
correcto en WS30 de Darwinex ($0.1/punto) y en cualquier otro broker.
"""
...
class OrderExecutor:
    ...
    MAGIC = 345681  # Bot NY=345678 | BTC=345679 | London FTMO=345680 | Darwinex=345681

    def __init__(self, symbol: str = "WS30"):
        self.symbol = symbol

    def _usd_per_point(self) -> float:
        """USD que gana/pierde 1 lote por cada 1.0 de movimiento de precio.
        Se lee de MT5 (tick_value / tick_size). Para WS30 Darwinex = 0.1;
    ...
    def execute_signal(self, signal, risk_usd: float, dry_run: bool = False) -> Tuple[bool, dict]:
        ...
        request = {
            ...
            "deviation":   0,
            "magic":       self.MAGIC,
            "comment":     f"OBD_{trade_type}_STOP",  # OBD = Order Block Darwinex
