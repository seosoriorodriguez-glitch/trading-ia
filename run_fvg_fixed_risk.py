# -*- coding: utf-8 -*-
"""
Recalcula el backtest FVG 518d con riesgo FIJO $50 (sin compounding).
pnl_r es invariante al balance: PnL real = pnl_r * $50
"""
import sys
...
INPUT = "output/fvg_live_backtest_518d.csv"


def main():
    df = pd.read_csv(INPUT, parse_dates=["entry_time", "exit_time"])
    ...
    print(f"{'='*65}")
    print(f"  Periodo:           {df['entry_time'].iloc[0].date()} -> {df['exit_time'].iloc[-1].date()}")
    ...
    target_hit = cumulative[cumulative >= 1000]
    if len(target_hit) > 0:
    ...
    times_over = (cumulative >= 1000).sum()
    print(f"  Trades sobre $1k:  {times_over} de {total_trades}")
    print(f"{'='*65}\n")
